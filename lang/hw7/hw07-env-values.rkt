...
(define-datatype expressed-val expressed-val?
  ...
  (step-val (s step?))
  (point-val (p point?))
  (proc-val (params symbols?) (body expr?) (env environment?))
  )
...
(define (proc-val->args fun)
  (or (expressed-val? fun) (invalid-args-exception "proc-val->args" "expressed-val?" fun))
  (cases expressed-val fun
    (proc-val (args body env) args)
    (else (invalid-args-exception "proc-val->args" "proc-val?" fun))))

(define (proc-val->body fun)
  (or (expressed-val? fun) (invalid-args-exception "proc-val->body" "expressed-val?" fun))
  (cases expressed-val fun
    (proc-val (args body env) body)
    (else (invalid-args-exception "proc-val->body" "proc-val?" fun))))

(define (proc-val->env fun)
  (or (expressed-val? fun) (invalid-args-exception "proc-val->env" "expressed-val?" fun))
  (cases expressed-val fun
    (proc-val (args body env) env)
    (else (invalid-args-exception "proc-val->env" "proc-val?" fun))))
