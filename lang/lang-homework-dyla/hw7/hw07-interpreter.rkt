...
(define (value-of-program prog env)
  (cases program prog
    (a-program
     (var-exprs expr rest-of-expressions)
     ...
     (let ([new-env (foldl value-of env var-exprs)])
       (andmap (lambda (ex) (value-of ex new-env))
  ...
  (cases expr ex
    ...
    (fun-call-expr
     ...
     (let
         ([fun (value-of f env)])
       (let
           ([args (proc-val->args fun)]
            [body (proc-val->body fun)]
            [f-env (proc-val->env fun)])
         (if
          (not (equal? (length args) (length params)))
          (raise
           (~a "arity mismatch. expected " (length args) " arguments, received " (length params)))
          (value-of
           body
           (create-env
            args
...
(define (value-of-var v-ex env)
  ...
  (cases var-expr v-ex
    ...
    (fun-def-expr
     (iden args body)
     (extend-env-wrapper iden (value-of (fun-expr args body) env) env FINAL))
